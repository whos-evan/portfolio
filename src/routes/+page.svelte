<script lang="ts">
  ...
  function spawnDialogBox(content: string, title: string, image?: string) {
    // create new draggable window
  ...
  function trolling() {
    ...
    document.body.style.color = `rgb(${Math.floor(
      Math.random() * 255
    )}, ${Math.floor(Math.random() * 255)}, ${Math.floor(
      Math.random() * 255
    )})`;
...
>
  ...
  <button
    class="m-5"
    use:draggable={{ axis: "both", bounds: "parent" }}
    on:dblclick={() => spawnDialogBox("", "trollface", "/images/trollface.png")}
    use:tap={{ timeframe: 300 }}
    on:tap={() => spawnDialogBox("", "trollface", "/images/trollface.png")}
    on:tap={() => trolling()}
  >
    <img
      ...
      class="w-16 h-16 m-auto grayscale bg-white"
    />
    <p class="bg-white text-center text-sm mt-2">trollface</p>
  </button>
</div>
